...
object HotAreaOrder {
  ...
  def _thrid_hotArea_order(sparkSession: SparkSession): Unit = {
    // 注册 UDF - 使用 udf 函数方式
    val locationToH3UDF = udf((lat: Double, lon: Double, res: Int) => {
      h3.geoToH3(lat, lon, res)
    })
    sparkSession.udf.register("locationToH3", locationToH3UDF)

    val h3ToCoordinateUDF = udf((line: String) => {
      val geo: GeoCoord = h3.h3ToGeo(line.toLong)
      geo.lat + "," + geo.lng
    })
    sparkSession.udf.register("h3ToCoordinate", h3ToCoordinateUDF)
    ...
    val data: RDD[(String, Iterable[(String, String, Int, Int)])] = today_hot_area_rdd.map { line =>
      //当日下单的街道经纬度编码
    ...
    val parseData: RDD[(String, Int, List[(String, String, Int, Int)])] = data.map { line =>
      var count = 0
      for (dat <- line._2) {
        count = count + dat._4
      }
    ...
    val _new_hot = yest_hot_new_status_rdd.map { line =>
      //昨日下单的街道经纬度编码
    ...
    //对每个街道热区情况计数
    val _new_hot_parse = _new_hot.map { line =>
      var count = 0
      for (dat <- line._2) {
        count = count + dat._4
      }
    ...
    SparkSQLHBaseSink.saveToHBase(_hot_order,"_hot_order","rk,_city_code,_day_order_num,_new_orders_num")
  }
}