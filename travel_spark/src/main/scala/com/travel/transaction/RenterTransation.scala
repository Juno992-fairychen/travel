...
object RenterTransation {
  ...
  def renter_register_active(session:SparkSession): Unit ={
    ...
    //结果落地
    SparkSQLHBaseSink.saveToHBase(summary_register_active,"summary_register_active","MM:rk,MM:_city_code,MM:_day_new_user,MM:DAU")
  }
  ...
  def plat_register_status(session: SparkSession): Unit ={
    ...
    SparkSQLHBaseSink.saveToHBase(palt_mwd,"palt_mwd","MM:myid,MM:monthNewUserCount,MM:weekNewUserCount,MM:dayNewUserCount")
  }
  ...
  def stateRate(session:SparkSession): Unit ={
    ...
    session.sql(RenterSQL.weekStateRate("2019-07-07" , "2019-07-15")).createOrReplaceTempView("weekStateRate")
    session.sql(RenterSQL.monthStateRate("2019-07-01" , "2019-07-30")).createOrReplaceTempView("monthStateRate")
    ...
    SparkSQLHBaseSink.saveToHBase(stateRate,"stateRage","MM:myid,MM:monthStateRate,MM:weekStateRate,MM:dayStateRate")
  }
  ...
  def active_user(session: SparkSession): Unit ={
    ...
    SparkSQLHBaseSink.saveToHBase(active,"active","MM:rk,MM:DAU,MM:WAU,MM:MAU")
