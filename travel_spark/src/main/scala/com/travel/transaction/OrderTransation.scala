...
object OrderTransation {
  ...
  private def orderMonitor(session:SparkSession): Unit ={
    ...
    SparkSQLHBaseSink.saveToHBase(final_summary_order, "final_summary_order",
      "MM:rk,MM:monthVehicleCount,MM:dayVehicleCount,MM:o_begin_address_code,MM:monthOrderCount,MM:weekOrderCount,MM:dayOrderCount")
  }
  ...
  def summarry_order(session:SparkSession): Unit ={
    ...
    SparkSQLHBaseSink.saveToHBase(
      summary_plat,
      "summary_plat",
      "MM:rk,MM:totalOrderNum,MM:monthOrderNum,MM:avgOrderNum,MM:charge_mileage"
    )
  }
  ...
  def order_renter_pay(sparkSession: SparkSession): Unit ={
    ...
    SparkSQLHBaseSink.saveToHBase(
      summary_order_register_pay,
      "summary_order_register_pay",
      "MM:myid,MM:totalCount,MM:registerTotalCount,MM:pay_all"
    )
  }
}
