...
object HotOrderTransation {
  ...
  def init(sparkSession: SparkSession): Unit ={
    ...
    lazy val orderHotTmp =(time:String) =>
      s"""
         |select
         | open_lng,
         | open_lat,
         ...
         | from order
         | where create_time >= to_date(cast(${time} as string), 'yyyyMMdd')
    """.stripMargin
    ...
    lazy val getHotArea =
      """
        ...
        |FROM (
        |  SELECT *,
        |    ROW_NUMBER() OVER (PARTITION BY h3Code ORDER BY create_time DESC) as rn
        |  FROM orderHotTmp
        |) t
        |WHERE rn = 1
  """.stripMargin
    ...
    SparkSQLHBaseSink.saveToHBase(hotOrder,"hotOrder","rk,begin_address_code,centerPoint,count")
