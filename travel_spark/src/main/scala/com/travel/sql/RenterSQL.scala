...
object RenterSQL {
  ...
  lazy val dayStateRate = (today:String , lastDay:String) =>
    s"""
       |SELECT
       |  1 as myid,
       |  concat(round(sum(case when t2.id is not null then 1 else 0 end) * 100.0 / count(t1.id), 2), '%') as dayStateRate
       |FROM
       |  (SELECT id FROM renter WHERE date_format(create_time, 'yyyy-MM-dd') = '${lastDay}') t1
       |LEFT JOIN
       |  (SELECT id FROM renter WHERE date_format(last_logon_time, 'yyyy-MM-dd') = '${today}') t2
       |ON t1.id = t2.id
    """.stripMargin
